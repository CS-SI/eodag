...
from .mappinginterpretor import MappingInterpretor
...
class Search(PluginTopic):
    ...
    def query(
        ...
        **kwargs: Any,
        ...
        which will be processed by a :class:`~eodag.plugins.download.protocol.base.Download` plugin (2) and the total
        number of products matching the search criteria. If ``prep.count`` is False, the second element returned must
        be ``None``.
        """
    ...
    def get_assets_from_mapping(
        ...
        properties = deepcopy(getattr(product, "properties", {}))
        ...
        if isinstance(assets_mapping, dict):
        ...
        if isinstance(imported_assets, list):
            buffer = {}
            index = 0
            for item in imported_assets:
                ...
                if key == "":
                    index += 1
                    key = "asset-{}".format(index)
                buffer[key] = item
            imported_assets = buffer
        ...
        if driver is not None:
            ...
            for asset_key in imported_assets:
                ...
                norm_key, roles = driver.guess_asset_key_and_roles(
