...
class ECMWFSearch(PostJsonSearch):
    ...
    def __init__(self, provider: str, config: PluginConfig) -> None:
        ...
        # Cache query parameters of previous searches to be able to rebuild them on search by ID
    ...
    def normalize_results(
        ...
        if searched_id := kwargs.get("id"):
            if searched_id in self._query_cache:
                sorted_unpaginated_qp = self._query_cache[searched_id]
        # if not search by ID: store query params in cache later when ID is generated
        ...
        if result:
            ...
            # Cache query params for a potential later search by ID
            if not searched_id:
                ...
                self._query_cache[generated_id] = sorted_unpaginated_qp

            current_id = searched_id or generated_id
            logger.debug(f"Is this a search by ID? {'Yes' if searched_id else 'No'}")
            logger.debug(
                f"All properties for ID {current_id}:\n{json.dumps(properties, indent=2, default=str)}"
            )
            logger.debug(
                f"Cached query params for ID {current_id}:\n"
                + f"{json.dumps(self._query_cache[current_id], indent=2, default=str)}"
            )
