...
class ECMWFSearch(PostJsonSearch):
    ...
    def __init__(self, provider: str, config: PluginConfig) -> None:
        ...
        # Keep regular search properties by generated ORDERABLE id so a following
        # search by id can expose the same search parameters in product properties.
        self._orderable_search_properties: dict[str, dict[str, Any]] = {}
    ...
    def normalize_results(
        ...
        if result:
            ...
            searched_id = kwargs.get("id")
            if searched_id:
                # Keep the requested id untouched for id-based searches.
                properties["title"] = properties["id"] = searched_id

                # If this id was generated by a previous regular search in the same
                # session, restore those parameters in returned properties.
                cached_properties = self._orderable_search_properties.get(searched_id)
                if cached_properties:
                    for key, value in cached_properties.items():
                        if (
                            key not in properties
                            or properties[key] == NOT_AVAILABLE
                            or properties[key] is None
                        ):
                ...
                properties["title"] = properties["id"] = generated_id

                # Cache parsed properties from regular search for a potential later
                # id-based search in the same plugin instance.
                self._orderable_search_properties[generated_id] = {
                    key: deepcopy(value)
