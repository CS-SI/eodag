...
class TestApisPluginEcmwfGroupApi(BaseApisPluginTest):
    def setUp(self):
        self.provider = "cop_cds"
        self.api_plugin = self.get_search_plugin(provider=self.provider)
        self.product_type = "ERA5_SL"
    ...
    def test_plugins_apis_cop_ecwmf_group_search_by_id(self, mock_request, mock_auth):
        ...
        result = self.api_plugin.do_search(id=product_id)
        ...
        self.assertEqual("successful", result[0]["orderStatus"])
        self.assertIn("request_params", result[0])
        req_params = result[0]["request_params"]
